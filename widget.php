	...
	<?php if (is_user_logged_in()) : ?>
		...
		<?php if ($wpdb -> get_row("SELECT * FROM " . $wpdb -> prefix . $Subscriber -> table . " WHERE `email` = '" . $current_user -> user_email . "' AND `user_id` = '" . $current_user -> ID . "'")) : ?>
			<div class="ui-state-highlight ui-corner-all">
				<p style="padding:3px;"><i class="fa fa-check"></i> <?php echo sprintf(__('You are already subscribed with email address %s. Go to the %s page to manage your subscription.', $this -> plugin_name), '<strong>' . $current_user -> user_email . '</strong>', '<a href="' . $this -> get_managementpost(true) . '">' . __('manage subscriptions', $this -> plugin_name) . '</a>'); ?></p>
			</div>
		<?php endif; ?>
	<?php endif; ?>
<?php endif; ?>
...
<form action="<?php echo $action; ?>" method="post" id="<?php echo $widget_id; ?>-form" class="newsletters-form">
	...
	<div id="<?php echo $widget_id; ?>-fields" class="newsletters-form-fields" style="padding:3px;">
		<?php 
	...
	<?php if ($captcha_type = $this -> use_captcha(__($instance['captcha']))) : ?>		
		<div class="newsletters-fieldholder newsletters-captcha" style="padding:3px;">
	...
	<div id="<?php echo $widget_id; ?>-submit" class="newsletters_submit" style="padding:3px; padding-left:100px;">
